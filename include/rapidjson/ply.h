...
public:
    ...
    PlyElement(const PlyElement &rhs) : property_order(rhs.property_order), colors(rhs.colors),
    ...
    PlyElement(const std::vector<std::string> &property_order0,
        ...
        size_t N = arr.size();
        size_t i = 0;
        for (std::vector<std::string>::const_iterator name = property_order0.begin();
             name != property_order0.end(); name++, i++) {
...
private:
    ...
    struct Number {
        ...
        Number(const uint16_t &flag) :
                i64(0), f(0.0), i8(0), u8(0), i16(0), u16(0), i32(0), u32(0), d(0.0) { this->assign(flag, 0); }
        ...
        std::ostream &write(const uint16_t &flag, std::ostream &out) const {
            switch (flag) {
                ...
                case (kUint8Flag) :
                    return u8.write(out);
                case (kInt16Flag) :
                    return i16.write(out);
                case (kUint16Flag) :
                    return u16.write(out);
                case (kInt32Flag) :
                    return i32.write(out);
                case (kUint32Flag) :
                    return u32.write(out);
                case (kFloatFlag) :
                    return f.write(out);
                case (kDoubleFlag) :
                    out << d;
        ...
        bool is_equal(const uint16_t &flag, const Number &y) const {
            switch (flag) {
                case (kInt8Flag) :
                    return i8.is_equal(y.i8);
                case (kUint8Flag) :
                    return u8.is_equal(y.u8);
                case (kInt16Flag) :
                    return i16.is_equal(y.i16);
                case (kUint16Flag) :
                    return u16.is_equal(y.u16);
                case (kInt32Flag) :
                    return i32.is_equal(y.i32);
                case (kUint32Flag) :
                    return u32.is_equal(y.u32);
                case (kFloatFlag) :
                    return f.is_equal(y.f);
                case (kDoubleFlag) :
                    return (abs(d - y.d) < 0.01);
                default:
                    RAPIDJSON_ASSERT(false);
            }
            return false;
    ...
    bool is_equal_data(const Data &d1, const Data &d2) const {
        ...
        if (d1.f & kListFlag) {
            ...
            for (std::vector<Number>::const_iterator it1 = d1.elements.begin(), it2 = d2.elements.begin();
                 it1 != d1.elements.end(); it1++, it2++) {
    ...
    void extend_aray_data_number(const Number &x, const uint16_t &flag,
        ...
        switch (flag) {
            case (kInt8Flag) :
                out.push_back((T) (x.i8.v));
                break;
            case (kUint8Flag) :
                out.push_back((T) (x.u8.v));
                break;
            case (kInt16Flag) :
                out.push_back((T) (x.i16.v));
                break;
            case (kUint16Flag) :
                out.push_back((T) (x.u16.v));
                break;
            case (kInt32Flag) :
                out.push_back((T) (x.i32.v));
                break;
            case (kUint32Flag) :
                out.push_back((T) (x.u32.v));
                break;
            case (kFloatFlag) :
                out.push_back((T) (x.f.v));
                break;
            case (kDoubleFlag) :
                out.push_back((T) (x.d));
                break;
...
public:
    ...
    bool is_equal(const PlyElement &rhs) const {
        ...
        for (std::map<std::string, Data>::const_iterator lit = this->properties.begin();
             lit != this->properties.end(); lit++) {
    ...
    std::ostream &write(std::ostream &out,
        ...
        for (std::vector<std::string>::const_iterator name = property_order.begin();
             name != property_order.end(); name++) {
            std::map<std::string, Data>::const_iterator it = properties.find(*name);
    ...
    void increase_index(const size_t N, size_t maxProp = 0) {
        size_t iProp = 0;
        for (std::vector<std::string>::iterator name = property_order.begin();
             name != property_order.end(); name++, iProp++) {
...
inline
bool operator==(const PlyElement &lhs, const PlyElement &rhs) { return lhs.is_equal(rhs); }
...
public:
    ...
    PlyElementSet(const std::string &name0 = "", const uint32_t &count0 = 0) :
            name(name0), count(count0), elements(), property_order(), colors(), property_flags(),
            property_size_flags() {}
    ...
    void set_flags(const std::vector<std::string> &property_names,
        ...
        for (typename std::vector<std::string>::const_iterator it = property_names.begin();
    ...
    bool requires_double() const {
        for (std::vector<std::string>::const_iterator iname = property_order.begin();
             iname != property_order.end(); iname++) {
...
//! \return true if the two instances are equivalent.
inline
bool operator==(const PlyElementSet &lhs, const PlyElementSet &rhs) { return lhs.is_equal(rhs); }
...
public:
    ...
    Ply(const Ply &rhs) : comments(rhs.comments), format(rhs.format), elements(rhs.elements),
                          element_order(rhs.element_order) {}
...
inline
bool operator==(const Ply &lhs, const Ply &rhs) { return lhs.is_equal(rhs); }
...
//! \return Output stream.
inline
std::ostream &operator<<(std::ostream &out, const Ply &p) { return p.write(out); }
...
//! \return Input sream.
inline
std::istream &operator>>(std::istream &in, Ply &p) { return p.read(in); }
